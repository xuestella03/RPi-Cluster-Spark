...
def plot_metrics(df, output_prefix='pidstat'):
    ...
    fig.suptitle(f'Query {tpch.config.CUR_QUERY} {tpch.config.CUR_CONFIG} SF{tpch.config.SF} exec mem{tpch.config.SPARK_EXECUTOR_MEMORY} {tpch.config.ACTIVE_CONFIG} (PID {executor_pid})', fontsize=16, fontweight='bold')
