...
def plot_from_csv(filename):
    ...
    # Add actual values on top of the memory bars
    for bar in bars1:
        yval = bar.get_height()
        ax1.text(bar.get_x() + bar.get_width()/2, yval + 5, round(yval, 2), ha='center', va='bottom', color='steelblue', fontsize=10)
    
    # Add actual values on top of the runtime bars
    for bar in bars2:
        yval = bar.get_height()
        ax2.text(bar.get_x() + bar.get_width()/2, yval + 0.5, round(yval, 2), ha='center', va='bottom', color='darkorange', fontsize=10)
